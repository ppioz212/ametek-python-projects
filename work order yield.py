...
df = pd.read_excel('2022 BB yield baseline.xlsx')
...
df.to_excel('2022 Yield Baseline BB edit.xlsx', encoding="utf-8", index=False)
df['Thickness'] = df['PART_ID'].map(df2.set_index('Part ID')['Thickness'])
df['Thickness'] = df['Thickness'].map(thickness_edit)
df['Width'] = df['PART_ID'].map(df2.set_index('Part ID')['Width'])
df.to_excel('2022 Yield Baseline BB edit.xlsx', encoding="utf-8", index=False)